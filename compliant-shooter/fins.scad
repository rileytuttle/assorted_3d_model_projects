...
depth = 15;
// multiple_fins(5, spacing=3, height=100, width=15, thickness=1, angle=10);
// fin_carrier();
// full_fin(100, 15, 1, angle=-10);
// back(50)
// half_fin(100, 15, 1, angle=-10);
difference() {
    chassis();
    back(31)
    right(45)
    down(depth/2)
    up(3)
    teardrop(d=depth*0.8, l=50, spin=-90, anchor=FRONT, ang=50)
        position(FRONT)
        cube([depth*0.8, 50, depth*0.8*0.5], anchor=FRONT+TOP);
}
diff() {
    back(31)
    right(32)
    down(depth/2)
    up(3)
    teardrop(d=depth*0.75, l=70, spin=-90, anchor=FRONT, ang=50)
        position(FRONT)
        cube([depth*0.75, 70, depth*0.75*0.5], anchor=FRONT+TOP)
        tag("remove")
        position(BOTTOM)
        up(3/2)
        up(1)
        #cube([depth, 71, 6], anchor=TOP);
}
